...
log4js.addLayout("json", function (config) {
  return function (logEvent) {
    return `[${JSON.stringify(
      logEvent.startTime
    )}] [${JSON.stringify(logEvent.level.levelStr)}] ${JSON.stringify(logEvent.data)} ${JSON.stringify(logEvent.callStack).split("\\n")[1].replace('"', "").replace("   ", "").replaceAll(String.fromCharCode(92, 92), String.fromCharCode(92))} `;
  };
});
log4js.configure({
  appenders: {
...
const sendResponseMsg = (res, message, success, code) => {
  if (!res.headersSent) return res.status(code).json({ message, success });
};
